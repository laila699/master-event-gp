...
const MemberSchema = new Schema<IMember>(
  ...
  { _id: true }  // يعني كل عضو له ObjectId خاص فيه
);
...
export interface ITrip extends Document {
  ...
  title: string;
  date: Date;
  venue: string; // مكان الحدث
  venueLocation?: { //اختياري، موقع جغرافي للحدث
    type: "Point";
...
TripSchema.index({ venueLocation: "2dsphere" }, { sparse: true });

TripSchema.virtual("remainingBudget").get(async function () {
  /* -------------------------------------------------------------
