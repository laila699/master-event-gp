...
const arToService: Record<string, VendorServiceType> = {
  ...
  المرشدين: VendorServiceType.Guides,
  الطعام: VendorServiceType.Restaurant,
 النقل: VendorServiceType.Transportation,
  الاقامة: VendorServiceType.Accommodation,
...
export const recommendedOfferings = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    const limit = parseInt((req.query.limit as string) || "5", 10); // Get limit from query, default to 5 للعروض يلي حنرجعها

    // 1) fetch event & ownership
    const ev = await Trip.findById(eventId);
    if (!ev) return res.status(404).json({ message: "Trip not found" });

    const user = req.user as { _id: any; role: string };
    if (
      user.role !== "admin" &&
      user._id.toString() !== ev.organizer.toString() // <— check if user is admin or organizer
    )
    ...
    for (const [catAr, cash] of Object.entries(remaining)) {
      ...
      const offers = await Offering.aggregate([ // جلب العروض من قاعدة البيانات
        // join vendor & filter by serviceType
        {
          $lookup: { // ربط العروض بالمزودين
            from: "users",
            localField: "vendor",
            foreignField: "_id",
            pipeline: [
              {
                $match: { // تصفية المزودين حسب نوع الخدمة
                  role: "vendor",
        ...
        {
          $addFields: { sortKey: { $subtract: [10, "$vendor.averageRating"] } }, // ترتيب حسب التقييم
        },
        { $sort: { sortKey: 1, price: 1 } },
        { $limit: limit }, // حدد عدد النتائج
        {
          $project: { // حدد الحقول التي سيتم إرجاعها
            id: "$_id",
            title: 1,
            price: 1,
      ...
      out.push({ category: catAr, remaining: cash, offers }); // أضف النتيجة إلى المصفوفة النهائية
    }
