...
export const sendChatNotification = asyncHandler(
  async (req: Request, res: Response) => {
    const { chatId, senderId, text, recipients } = req.body as {
      ...
      senderId: string;
      text: string;
      recipients: string[]; // قائمة معرفات المستخدمين المستلمين
    };
...
export const saveFcmToken = asyncHandler(async (req, res) => {
  ...
  /* 1️⃣  Add token to fcmTokens array */
...
export const removeFcmToken = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    user.fcmTokens = user.fcmTokens.filter((t) => t !== token); // remove from user's fcmTokens
    await user.save();
