...
export const listVendorBookings = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    const offeringIds = offerings.map((o) => o._id); // جلب معرفات العروض
...
export const createVendorRating = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    await (vendor as any).addRating({
      ...
      value,
      review,
      ratedAt: new Date(), // when the rating was made
    });
...
export const listVendors = asyncHandler(async (req: Request, res: Response) => {
  ...
  const filter: any = { role: "vendor" }; // start with role=vendor
  ...
  // 3) geolocation
  if (lat && lng) {
    const latitude = parseFloat(lat as string); // تحويل النص إلى رقم عشري
    const longitude = parseFloat(lng as string);
    const km = radius ? parseFloat(radius as string) : 10; // default to 10 km
...
export const uploadAttributeImage = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    const attrs = user.vendorProfile?.attributes; //attributes iss array
    if (!attrs) {
      return res
        .status(400)
  ...
  async (req: Request, res: Response) => {
    ...
    if (!user.vendorProfile) {
      user.vendorProfile = {
        ...
        serviceType: VendorServiceType.Accommodation,
        bio: "",
