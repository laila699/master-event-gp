...
router.put(
  "/:vendorId",
  requireAuth,
  requireRole("vendor"), // Ensure the user is a vendor
  updateVendorAttributes
);
router.post(
  "/:vendorId/attributes/:key/image", // key : the attribute key to which the image belongs
  requireAuth,
  requireRole("vendor"),
  uploadAttributeImages.single("file"), // upload an image for a vendor attribute
  uploadAttributeImage
);
router.post("/:vendorId/ratings", requireAuth, createVendorRating);

router.get("/:vendorId/bookings", requireRole("vendor"), listVendorBookings); // List bookings for a vendor
