...
import '../screens/my_events_screen.dart'; //for organizer role
...
class DashboardScreen extends ConsumerStatefulWidget { //  access to Riverpod ref in state
  final User user;
...
class _DashboardScreenState extends ConsumerState<DashboardScreen> {
  @override 
  void initState() {  //تشغل مرة واحدة فقط عند انشاء الستيت 
  ...
  Future<void> _requestLocationPermission() async {
    ...
    if (status.isGranted) { // إذا تم منح الإذن
      print("📍 Location permission granted.");
    } else if (status.isDenied) {
      print("❗ Location permission denied.");
    } else if (status.isPermanentlyDenied) {
      print( // إذا تم رفض الإذن بشكل دائم
  ...
  @override
  Widget build(BuildContext context) { //build method تحدد اي شاشة
