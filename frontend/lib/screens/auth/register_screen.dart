...
class _RegisterScreenState extends ConsumerState<RegisterScreen> {
  ...
  final _formKey = GlobalKey<FormState>(); // FormState كلاس بمثل حالة النموذج
  ...
  Future<void> _pickImage(ImageSource src) async {
    try {
      ...
      if (mounted) // check if the widget is still
  ...
  void _showImageSourceSheet() => showModalBottomSheet(
    ...
    builder:
        (_) => SafeArea(
          child: Wrap(
            // Wrap to allow scrolling if needed
            children: [
              ListTile(
                leading: const Icon(
                  Icons.photo_library,
                ...
                onTap: () {
                  Navigator.pop(
                    context,
                  ); // Close the bottom sheet image source selection
                  _pickImage(
                    ImageSource.gallery,
