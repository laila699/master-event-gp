...
class NotificationItem {
  NotificationItem({
    ...
    required this.body,
    required this.data,
  }) : receivedAt = DateTime.now(); // حفظ وقت الاستلام
...
final notificationServiceProvider = Provider((ref) {
  ...
  svc.init();
  return svc; // يمكن استخدامه في أي مكان في التطبيق
});


class NotificationService {
  NotificationService._(this._ref); // يمنع انشاء الخدمة من خارج الكلاس 

  final ProviderRef _ref; // يستخدم للوصول الى مزودات Riverpod
  final _messaging = FirebaseMessaging.instance; // fcm
    ...
    if (Platform.isAndroid) { 
      final impl =
          _local 
              ...
              >(); //الحصول على نسخة خاصة بأندرويد من LocalNotificationsPlugin:
  ...
  Future<void> _initFCM() async {
    final perm = await _messaging.requestPermission(); // طلب إذن الإشعارات
    if (perm.authorizationStatus != AuthorizationStatus.authorized) return;

    /* ─ Register / refresh token with your backend ─ */
    await _registerToken();
    _messaging.onTokenRefresh.listen(_registerToken); // تحديث التوكن عند تغييره
...
// send fcm token to backend
  ...
  void _handleMessage(RemoteMessage msg) {
    final n = msg.notification; //يتحقق إذا كانت الرسالة تحتوي على إشعار مرئي (notification).
