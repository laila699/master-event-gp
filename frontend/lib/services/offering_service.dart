...
class OfferingService {
  final Dio _dio;
  final TokenStorage _tokenStorage; // Dependency for token management
  ...
  Future<List<Offering>> fetchAllOfferings({
    VendorServiceType? serviceType, // نوع الخدمة الاختياري
  }) async {
    // build query-params
    final qp = <String, dynamic>{};
    if (serviceType != null) {
      // our backend expects the enum key, not an ObjectId
      qp['serviceType'] = serviceType.value; // بضيفه على query params
    }

    final resp = await _dio.get('/offerings', queryParameters: qp); // استدعاء جميع العروض
  ...
  Future<Offering> fetchOfferingById({
    ...
    final resp = await _dio.get('/vendors/$vendorId/offerings/$offeringId'); // استدعاء عرض محدد
  ...
  Future<Offering> createOffering({ 
    ...
    List<File>? imageFiles, //مجموعة من الملفات التي سيتم تحميلها
  }) async {
    final formDataMap = {  // تجهيز البيانات للإرسال
      'title': title,
      'price': price,
    ...
    if (imageFiles != null && imageFiles.isNotEmpty) {
      ...
      for (var file in imageFiles) {
        ...
        uploaded.add(
          await MultipartFile.fromFile(file.path, filename: filename), // تحميل الملف
        );
      }
      formDataMap['images'] = uploaded; // إضافة الصور إلى البيانات
    }

    final formData = FormData.fromMap(formDataMap); // تحويل البيانات إلى FormData
    print('Creating offering with data: $formDataMap');
    final response = await _dio.post( // يبعت طلب ل backend عشان ينشئ عرض جديد
      '/vendors/$vendorId/offerings',
      data: formData,
      options: Options(contentType: 'multipart/form-data'), // تحديد نوع المحتوى
    );
  ...
  Future<Offering> updateOffering({
    ...
    if (newImageFiles != null && newImageFiles.isNotEmpty) {
      final uploaded = <MultipartFile>[]; // create a list to hold the uploaded files
      for (var file in newImageFiles) {
        final filename = file.path.split('/').last; // extract the filename from the file path
        uploaded.add(
