...
class ProviderAttribute {
  ...
  final AttrType type; 
  dynamic value;        // dynamic value can hold any type
  bool required; 
  List<dynamic>? options; // for select/multiSelect options
  String? itemType; // for array items
  List<ProviderAttribute>? fields; // for object attributes
  ...
  factory ProviderAttribute.fromJson(Map<String, dynamic> j) {    
    final t = AttrType.values.firstWhere((e) => e.name == j['type']); // get type
    return ProviderAttribute( //create a new ProviderAttribute from JSON
      key: j['key'],
      label: j['label'] ?? j['key'],
      type: t,
      value: j['value'],
